...
fn zoom_config(screen_width: u32, screen_height: u32) -> (Viewport, Projection) {
...
fn minimap_config(screen_width: u32, _screen_height: u32) -> (Viewport, Projection) {
...
fn control(input: Res<ButtonInput<KeyCode>>, mut q_transform: Query<&mut Transform, With<Zoom>>) {
    let mut transform = q_transform.single_mut().unwrap();
    let step = 20.0;
